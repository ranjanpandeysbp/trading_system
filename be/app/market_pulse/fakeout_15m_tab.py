...
fakeout_15m_tab.py
-----------------
1min – 15min Breakout scanner **live screener** for Groww & CoinDCX.
...
- APPROACHING_HIGH/LOW: price near 15M range edge — breakout may start soon
- MONITORING: inside range, no imminent setup

Groww: IST 09:15–09:30 range, signals 09:30–15:30 IST. CoinDCX: NY 00:00–00:15 range.
...
def fetch_1m_screener_data(
    ...
    """Fetch recent 1M OHLCV for live screener (today + prior session)."""
    df = fetch_data_for_gap_scan(
        symbol, "1m", market, groww_token, exchange, limit=lookback_bars,
    )
...
def _build_screener_chart(df_1m: pd.DataFrame, analysis: dict, symbol: str) -> go.Figure:
    """1M candlesticks with 15M range window, TOP/BOTTOM lines, and signal zone."""
    ...
    range_label = sc.get("range_label") or ("09:15–09:30 IST" if session_mode == "india" else "00:00–00:15 NY")
    ...
    # 15M range formation window (shaded)
    ...
    # 15M candle body (open → close) inside the range window
    if (
    ...
    # 15M TOP / BOTTOM — full-width reference lines + bold segment after range completes
    if rh is not None and rl is not None:
        for y_val, color, label in (
            (rh, "#ef4444", "15M TOP"),
            (rl, "#10b981", "15M BOTTOM"),
        ):
            fig.add_hline(
                y=y_val,
                line_dash="dot",
    ...
    # Vertical marker: signal window starts after 15M range completes
    ...
    fig.update_layout(
        title=dict(
            text=f"{symbol} — 1M screener{title_suffix}",
            font=dict(size=14),
        ),
...
def _render_priority_screener_charts(results: dict, is_crypto: bool, *, max_charts: int = 3) -> None:
    ...
    st.markdown("### 📈 15M Range Chart — Top / Bottom Lines")
    st.caption(
        "Blue zone = 15M range forming · **Red** = 15M TOP (high) · **Green** = 15M BOTTOM (low) · "
...
def _render_result_block(
    ...
    *,
    ...
    def _body() -> None:
        render_run_summary(summarize_fakeout_15m(analysis, symbol))
        ...
        render_strategy_mtf_panel(
            ...
            exchange=st.session_state.get("f15m_results_exchange", "NSE"),
            primary_tf="1m",
        ...
        session_mode = analysis.get("session_mode", "ny")
        range_title = (
            "15M Range (IST · 09:15–09:30)"
            if session_mode == "india"
            else "15M Range (NY Session)"
        )
        ...
        st.markdown("### 📈 1M Chart · 15M TOP / BOTTOM Lines")
        st.plotly_chart(
        ...
        if plan and sc.get("primary_phase") == "ENTRY_READY":
            render_demo_trade_panel(
                ...
                source_tab="1min-15min Breakout scanner",
                groww_token=get_active_groww_token(),
                exchange=st.session_state.get("f15m_results_exchange", "NSE"),
            )
...
def render_fakeout_15m_tab():
    """1min – 15min Breakout scanner live screener."""
    st.markdown("<h1>⚡ 1min – 15min Breakout scanner</h1>", unsafe_allow_html=True)
    st.write(
        "Scans **multiple tickers** on **1M** for approaching and active fakeout setups. "
        "**Groww:** 15M range **09:15–09:30 IST**, signals **09:30–15:30 IST**. "
        "**CoinDCX:** NY 15M range. Alerts when breakouts, fakeouts, or edge approaches are detected."
    )
    ...
    m1, m2 = st.columns(2)
    with m1:
        f15m_market = st.selectbox(
            "🌐 Market",
            MARKET_OPTIONS,
            key="f15m_market",
        )
    with m2:
        f15m_exchange = st.selectbox("Exchange", ["NSE", "BSE"], index=0, key="f15m_exchange") if is_india_market(f15m_market) else "NSE"

    st.markdown("### 📊 Tickers")
    f15m_tickers: list[str] = []
    if is_crypto_market(f15m_market):
        f15m_tickers = render_coindcx_ticker_selection("f15m")
    else:
        f15m_tickers = render_equity_index_ticker_selection(f15m_market, "f15m")

    is_crypto = is_crypto_market(f15m_market)

    from app.market_pulse.ta_mtf_hub_ui import render_ta_fixed_scan_note

    render_ta_fixed_scan_note("1m", engine_label="1min–15min breakout engine")

    st.markdown("### ⚙️ Screener Parameters")
    tfc1, tfc2, tfc3, tfc4 = st.columns(4)
    with tfc1:
        f15m_rr = st.number_input("Reward : Risk", min_value=1.0, max_value=5.0, value=2.0, step=0.5, key="f15m_rr")
    with tfc2:
        f15m_approach = st.slider("Approaching edge (× range)", 0.05, 0.35, 0.15, 0.05, key="f15m_approach")
    with tfc3:
        f15m_bars = st.slider("1M bars to load", 200, 600, 400, 50, key="f15m_bars")
    with tfc4:
        f15m_actionable_only = st.checkbox("Show actionable only", value=False, key="f15m_actionable_only")

    with st.expander("⚙️ Advanced", expanded=False):
        f15m_large_brk = st.slider("Large breakout threshold (× range)", 0.2, 1.0, 0.5, 0.1, key="f15m_large_brk")
        f15m_max_sl = st.number_input(
            "Max SL % of entry",
            min_value=0.001, max_value=0.02, value=0.005, step=0.001, format="%.3f",
            key="f15m_max_sl",
        )
        f15m_auto_refresh = st.checkbox("Auto-refresh scan (2 min)", value=False, key="f15m_auto_refresh")

    total = len(f15m_tickers)
    session_hint = "IST 09:15–15:30" if not is_crypto else "NY 15M range"
    st.caption(f"🧮 **{total}** ticker(s) · live 1M screener · {session_hint}")
    ...
    with col_time:
        st.caption(f"Last scan: {st.session_state.get('f15m_last_scan', '—')}")

    if run_btn or st.session_state.get("f15m_auto_refresh_trigger"):
        st.session_state.pop("f15m_auto_refresh_trigger", None)
        if not f15m_tickers:
        ...
        groww_token = get_active_groww_token()
        session_mode = session_mode_for_market(f15m_market)
        for i, tick in enumerate(f15m_tickers):
            bar.progress((i + 1) / total, text=f"{tick} — scanning 1M…")
            try:
                df = fetch_1m_screener_data(
                    tick, f15m_market, groww_token, f15m_exchange, f15m_bars,
                )
                if df.empty or len(df) < 25:
                    results[tick] = {
                        "error": f"Insufficient 1M data ({len(df)} bars).",
                        "symbol": tick,
                    }
                    continue
                analysis = run_fakeout_screener(
                    df,
                    rr_ratio=f15m_rr,
                    large_breakout_threshold=f15m_large_brk,
                    max_sl_pct=f15m_max_sl,
                    session_mode=session_mode,
                    approach_pct=f15m_approach,
                )
            ...
            time.sleep(0.04)
        bar.empty()
        st.session_state.f15m_results = results
        st.session_state.f15m_results_market = f15m_market
        st.session_state.f15m_is_crypto = is_crypto
        st.session_state.f15m_results_exchange = f15m_exchange
        st.session_state.f15m_last_scan = datetime.now().strftime("%H:%M:%S")

    results = st.session_state.get("f15m_results", {})
    market_disp = st.session_state.get("f15m_results_market", f15m_market)
    is_crypto = st.session_state.get("f15m_is_crypto", is_crypto)
    ...
    for tick, d in results.items():
        if "error" in d:
            digest.append(summarize_error(d["symbol"], "1m", d["error"], tab="1min-15min Breakout scanner"))
        else:
            digest.append(summarize_fakeout_15m(d["analysis"], d["symbol"]))

    if f15m_actionable_only:
    ...
    for ti, ticker in enumerate(tickers_sorted):
        data = results[ticker]
        if f15m_actionable_only and "error" not in data:
    ...
    if results:
        ...
        snapshot_section_for_ask_ai("fakeout_15m")

    if st.session_state.get("f15m_auto_refresh"):
        time.sleep(120)
        st.session_state.f15m_auto_refresh_trigger = True
        st.rerun()

    st.caption(
        "⚠️ Screener flags approaching setups — always confirm on 1M chart before entry."
    )
