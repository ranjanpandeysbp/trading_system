...
export const runFallingKnifeScan = (payload: {
  ...
  exchange?: string
  mode?: 'live' | 'history' | 'from_top' | 'runup_descent'
  from_date?: string
  to_date?: string
  move_side?: 'fall' | 'rise' | 'both' | 'runup' | 'descent'
...
export const fetchTickerSuggestions = (asset_class: string, q: string, limit = 20) =>
  api
    .get<{ tickers: string[]; items?: Array<{ symbol: string; name: string; label: string }> }>(
      '/command-center/ticker-suggestions',
      { params: { asset_class, q, limit } },
    )
    .then((r) => r.data)
...
export const runProTradeEma9VolRsiMomentum = (payload: {
  tickers: string[]
  asset_class?: string
  exchange?: string
  entry_tf?: string
  ...
  require_htf_align?: boolean
  avoid_chop?: boolean
  skip_huge_candle?: boolean
  take_confidence_threshold?: number
  use_ai?: boolean
}) => api.post('/pro-trade/ema9-vol-rsi-momentum', payload, { timeout: MP_TIMEOUT }).then((r) => r.data)

export const runProTradeFlatRetest = (payload: {
  tickers: string[]
  asset_class?: string
  exchange?: string
  timeframes: string[]
  lookback_bars?: number
  flat_min_bars?: number
  flat_max_bars?: number
  flat_atr_mult?: number
  vol_expand_mult?: number
  min_rr?: number
  take_confidence_threshold?: number
  use_ai?: boolean
