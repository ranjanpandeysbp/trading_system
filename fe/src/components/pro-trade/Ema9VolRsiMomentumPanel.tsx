...
function TickerResultCard({
  ...
  const buyScore = Number(result.buy_score ?? 0)
  const sellScore = Number(result.sell_score ?? 0)
  ...
  const series = useMemo<VpSeries[]>(() => {
    ...
    return fromApi.length ? fromApi : [{ key: 'ema9', label: 'EMA 9', color: '#fbbf24' }]
  }, [result.chart_series])
        ...
        <button type="button" onClick={() => setOpen((o) => !o)} className="flex flex-1 flex-wrap items-center gap-3 text-left">
          ...
          {result.timeframe != null && (
            <span className="rounded-md border border-slate-700/60 bg-slate-800/50 px-1.5 py-0.5 text-[11px] text-slate-400">
              ...
              {result.trend_tf != null ? ` · HTF ${String(result.trend_tf)}` : ''}
            </span>
          )}
          {(result.ltp != null || metrics.price != null) && (
            <span className="text-sm text-slate-400">
              {currency}
              {fmtNum(result.ltp ?? metrics.price)}
            </span>
          )}
          <SignalBadge signal={String(result.signal ?? 'WAIT')} />
          <span className="rounded-md border border-slate-700/60 bg-slate-800/40 px-2 py-0.5 text-[11px] text-slate-300">
            B{buyScore} / S{sellScore}
          </span>
      ...
      {open && !result.error && (
        <div className="space-y-3 border-t border-slate-800/70 px-4 py-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-5">
            ...
            <div>
              <p className="text-xs text-slate-500">T1 / T2 / T3</p>
              <p className="font-medium text-emerald-400">
                {fmtNum(result.target_price)} · {fmtNum(result.target2_price)} · {fmtNum(result.target3_price)}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500">EMA9 · RSI</p>
              <p className="font-medium text-slate-200">
                {fmtNum(metrics.ema9)} · {fmtNum(metrics.rsi, 1)}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Vol / SMA50 · HTF</p>
              <p className="font-medium text-slate-200">
                {fmtNum(metrics.vol, 0)} / {fmtNum(metrics.vol_sma50, 0)} · {String(metrics.htf_bias ?? '—')}
              </p>
            </div>
          </div>
          ...
          {showCharts && chartData.length > 0 && (
            <VolumeProfileChart
              ...
              series={series}
              levels={levels}
              readingGuide="Yellow = 9 EMA. BUY = ≥4 closes above EMA + HH/HL + vol>SMA50 + RSI rising + break prior high. SELL = mirror with LH/LL + break prior low. Skip chop / huge chase candles."
            />
          )}

          <AskAIPanel
            context={String(result.ai_context ?? '')}
            section={`pro-trade/ema9-vol-rsi-momentum/${String(result.ticker ?? '')}`}
...
export function Ema9VolRsiMomentumPanel({ data, showCharts = false }: { data: Row; showCharts?: boolean }) {
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
        <span>
          ...
          {data.entry_tf != null && data.trend_tf != null
            ? ` · ${String(data.entry_tf)} / HTF ${String(data.trend_tf)}`
            : ''}
        </span>
