...
interface SavedReport {
  ...
  asset_class: string
  created_at: string
  summary?: { result_count?: number; domain?: string; section?: string }
}
...
export function useAnalysisBackground(domain: AnalysisDomain, section: string, enabled = true) {
  ...
  const runningJobsQuery = useQuery({
    queryKey: ['analysis-jobs-running', domain, section],
    queryFn: () => fetchAnalysisJobs(domain, section, 'running'),
  ...
  const recentJobsQuery = useQuery({
    queryKey: ['analysis-jobs-recent', domain, section],
    queryFn: () => fetchAnalysisJobs(domain, section, 'all'),
    enabled: enabled && !!section,
    refetchInterval: 5000,
  })

  const reportsQuery = useQuery({
    queryKey: ['analysis-reports', domain, section],
    queryFn: () => fetchAnalysisReports(domain, section),
    enabled: enabled && !!section,
  })

  useEffect(() => {
    if (!enabled) return
    const jobs = ((runningJobsQuery.data as { jobs?: BgJobStatus[] } | undefined)?.jobs) ?? []
    const ids = jobs.map((j) => j.job_id).filter(Boolean)
    if (!ids.length) return
    setBgJobIds((prev) => Array.from(new Set([...ids, ...prev])))
  }, [runningJobsQuery.data, enabled])

  const jobQueries = useQueries({
    queries: bgJobIds.map((id) => ({
      queryKey: ['analysis-job', domain, section, id],
      queryFn: () => fetchAnalysisJob(id) as Promise<BgJobStatus>,
      enabled: enabled && !!section,
  ...
  useEffect(() => {
    ...
    if (changed) {
      ...
      queryClient.invalidateQueries({ queryKey: ['analysis-jobs-running', domain, section] })
      queryClient.invalidateQueries({ queryKey: ['analysis-jobs-recent', domain, section] })
    }
  }, [bgJobIds, jobById, queryClient, domain, section, enabled])
  ...
  const recentFinished = recentJobs.filter((j) => j.status === 'done' || j.status === 'error').slice(0, 8)

  const reportDetailQuery = useQuery({
    queryKey: ['analysis-report', domain, section, viewedReportId],
    queryFn: () => fetchAnalysisReport(domain, section, viewedReportId as number),
    enabled: enabled && viewedReportId != null,
  })
  ...
  const startMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      startAnalysisJob({ ...payload, domain, section }),
    onSuccess: (data) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (reportId: number) => deleteAnalysisReport(domain, section, reportId),
...
export type AnalysisBgHook = ReturnType<typeof useAnalysisBackground>

export function AnalysisBackgroundControls({
  bg,
  onStart,
  placeholder,
}: {
          ...
          <div className="min-w-[16rem] flex-1">
            <FormField label="Report name">
              <Input
                ...
                placeholder={placeholder || `Analysis · ${new Date().toLocaleDateString()}`}
                maxLength={200}
              />
            </FormField>
          </div>
...
export function AnalysisBackgroundJobsAndReports({ bg }: { bg: AnalysisBgHook }) {
  return (
    <>
          ...
          {bg.reports.map((r) => (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                {r.summary?.result_count != null && (
                  <p className="mt-1 text-xs text-teal-400/90">
                    {r.summary.result_count} result{r.summary.result_count === 1 ? '' : 's'}
                  </p>
                )}
              </div>
